...
class Mynotes extends CI_Controller {
 ...
 function __construct(){
  parent::__construct();
  $this->load->model('mynotes_model', 'homes/lihat_data');
  $this->load->library('calendar', $this->_setting());
 }
  
 function index(){
  redirect('homes/lihat_data');
 }
 ...
 function do_add($year, $mon, $day){
  $this->homes/lihat_data->addNote($year, $mon, $day, $this->input->post('note', true));
  $this->notes($year, $mon);
 }
 ...
 function updel_note($date){
  $data = array(
     ...
     'month' => $this->_month($this->input->post('mon')),
     'year'  => $this->input->post('year'),
     'note'  => $this->homes/lihat_data->getNote($this->input->post('year'), $this->input->post('mon'), $date)
    );
 ...
 function edit_note($year, $mon, $day){
  $this->homes/lihat_data->editNote($year, $mon, $day, $this->input->post('note', true));
  $this->notes($year, $mon);
 }
  
 // aksi untuk hapus catatan
 function delete_note($year, $mon, $day){
  $this->homes/lihat_data->deleteNote($year, $mon, $day);
  $this->notes($year, $mon);
 }
 ...
 function notes($year = null, $mon = null){
  ...
  $date = $this->homes/lihat_data->getCalendar($year, $mon);
  $data = array(
     'notes' => $this->calendar->generate($year, $mon, $date),
     'year'  => $year,
     'mon'   => $mon
    );
  $this->load->view('homes/lihat_data', $data);
 }
