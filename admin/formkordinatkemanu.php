        ...
        <div class="panel-body">
          <form>
            <div class="form-group">
              ...
              <select class="form-control" name="id_manusia" id="id_manusia">
                <option value="">pilih program</option>
                <?php foreach ($itemmanusia->result() as $manusia) {
                  ?>
                  <option value="<?php echo $manusia->id_manusia;?>" ><?php echo $manusia->program;?></option>
                  <?php
                } ?>
              </select>
            </div>
          ...
          <tbody id="daftarmarkerkema">
            ...
            foreach ($itemkordinat->result() as $kordinat) {
              ...
              <tr>
                ...
                <td><button type="button" id="viewmarker" data-idkordimanusia="<?php echo $kordinat->id_kordimanusia; ?>" class="btn btn-sn btn-info" name="button" title="view marker kemanusian"><span class="glyphicon glyphicon-eye-open"></span> </button>
                  <button type="button" id="deletemarker" data-idkordimanusia="<?php echo $kordinat->id_kordimanusia; ?>" class="btn btn-sn btn-danger" name="button" title="delete marker kemanusian"><span class="glyphicon glyphicon-trash"></span> </button>
                </td>
              </tr>
              <?php
              $no++;
...
var map;
 ...
 .on('click','#reset',reset);
  ...
  function simpan(){
    var datamarkerman = {'id_manusia':$('#id_manusia').val(),
    'latitude':$('#latitude').val(),
    'longitude':$('#longitude').val()};console.log(datamarkerman);
    $.ajax({
      url : '<?php echo site_url('admin/kordinatkemanusian/create');?>',
      data : datamarkerman,
      dataType : 'json',
      type : 'POST',
      success : function(data,status){
        if (data.status!='error') {
          reset();
          $('#daftarmarkerkema').load('<?php echo current_url()." #daftarmarkerkema > *"; ?>');
        }else {
          alert(data.msg)
        }
      },
  ...
  function viewmarker(){
    var id = $(this).data('idkordimanusia');
    var datamarkerman = {'id_kordimanusia':id};console.log(datamarkerman);
    $.ajax({
      url : '<?php echo site_url('admin/kordinatkemanusian/read');?>',
      data : datamarkerman,
      dataType : 'json',
      type : 'POST',
      success : function(data,status){
  ...
  function deletemarker(){
    if (confirm("apakah anda ingin menghapus data ini?")) {
      ...
      var datamarkerman = {'id_kordimanusia':id};console.log(datamarkerman);
      $.ajax({
        url : '<?php echo site_url('admin/kordinatkemanusian/delete');?>',
        data : datamarkerman,
        dataType : 'json',
        type : 'POST',
        success : function(data,status){
