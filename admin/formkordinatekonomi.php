        ...
        <div class="panel-body">
          <form>
            ...
            <div class="form-group">
              ...
              <select class="form-control" name="id_ekonomi" id="id_ekonomi">
                <option value="">pilih program</option>
                <?php foreach ($itemekonomi->result() as $ekonomi) {
                  ?>
                  <option value="<?php echo $ekonomi->id_ekonomi;?>" ><?php echo $ekonomi->program;?></option>
                  <?php
                } ?>
              </select>
            </div>
          ...
          <tbody id="daftarmarkereko">
            ...
            foreach ($itemkordinat->result() as $kordinat) {
              ...
              <tr>
                ...
                <td><button type="button" id="viewmarker" data-idkordiekonomi="<?php echo $kordinat->id_kordiekonomi; ?>" class="btn btn-sn btn-info" name="button" title="view marker ekonomi"><span class="glyphicon glyphicon-eye-open"></span> </button>
                  <button type="button" id="deletemarker" data-idkordiekonomi="<?php echo $kordinat->id_kordiekonomi; ?>" class="btn btn-sn btn-danger" name="button" title="delete marker ekonomi"><span class="glyphicon glyphicon-trash"></span> </button>
                </td>
              </tr>
              <?php
              $no++;
...
var map;
 ...
 .on('click','#reset',reset);
  ...
  function simpan(){
    var dataekonomi = {'id_ekonomi':$('#id_ekonomi').val(),
    'latitude':$('#latitude').val(),
    'longitude':$('#longitude').val()};console.log(dataekonomi);
    $.ajax({
      url : '<?php echo site_url('admin/Kordinatekonomi/create');?>',
      data : dataekonomi,
      dataType : 'json',
      type : 'POST',
      success : function(data,status){
        if (data.status!='error') {
          reset();
          $('#daftarmarkereko').load('<?php echo current_url()." #daftarmarkereko > *"; ?>');
        }else {
          alert(data.msg)
        }
      },
  ...
  function viewmarker(){
    var id = $(this).data('idkordiekonomi');
    var datamarker = {'id_kordiekonomi':id};console.log(datamarker);
    $.ajax({
      url : '<?php echo site_url('admin/kordinatekonomi/read');?>',
      data : datamarker,
      dataType : 'json',
      type : 'POST',
      success : function(data,status){
  ...
  function deletemarker(){
    if (confirm("apakah anda ingin menghapus data ini?")) {
      ...
      var datamarker = {'id_kordiekonomi':id};console.log(datamarker);
      $.ajax({
        url : '<?php echo site_url('admin/kordinatekonomi/delete');?>',
        data : datamarker,
        dataType : 'json',
        type : 'POST',
        success : function(data,status){
